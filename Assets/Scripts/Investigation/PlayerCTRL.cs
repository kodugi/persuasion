...
{
    public class PlayerCTRL : Utility
    {
        ...
        bool isFootStepLeft;
        private InteractionCTRL interactionScript;
        ...
        void Start()
        {
            interactionScript = GameObject.FindFirstObjectByType<InteractionCTRL>().GetComponent<InteractionCTRL>();
        ...
        void FixedUpdate()
        {
            ...
            {
                if(gameObject.transform.position.y <= obj.transform.position.y + obj.GetComponent<InteractionObj>().hideCriteria)
                {
        ...
        {
            return (isHiding && obj.GetComponent<InteractionObj>() is InteractionObj_Hidable);
        }
        private void OnTriggerEnter2D(Collider2D collision)
        {
            if (collision.gameObject.CompareTag("Inv_Interactable"))
            {
                ...
                if(collision.GetComponent<InteractionObj>().manuallyTouchable)
                {
                    if(AlreadyHiding(collision.gameObject))
                    {
                        //nothing
                    }
                    else interactionScript.QueueInteraction(collision.GetComponent<InteractionObj>().obj_name, true);
        ...
        private void OnTriggerExit2D(Collider2D collision)
        {
            if (collision.gameObject.CompareTag("Inv_Interactable"))
            {
                ...
                if(collision.GetComponent<InteractionObj>().manuallyTouchable)
                {
                    interactionScript.QueueInteraction(collision.GetComponent<InteractionObj>().obj_name, false);
