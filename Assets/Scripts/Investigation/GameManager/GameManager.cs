    ...
    public partial class GameManager : Utility
    {
        ...
        IEnumerator SetScene()
        {
            ...
            foreach(InteractableObj obj in objects)
            {
                ...
                {
                    interactable.AddComponent<InteractionObj_Hidable>();
                    interactable.GetComponent<InteractionObj_Hidable>().SetName(obj.title);
                }
                ...
                {
                    interactable.AddComponent<InteractionObj>();
                }
                interactable.GetComponent<InteractionObj>().state = -1;
                interactable.GetComponent<InteractionObj>().hideCriteria = obj.hideCriteria;
                interactable.GetComponent<InteractionObj>().manuallyTouchable = obj.manually_touchable;
                interactable.GetComponent<InteractionObj>().images = obj.image;
                ...
                //interactable.GetComponent<InteractionObj>().singleImage = obj.singleImage;
                if (Mathf.Abs(obj.colliderSize.x) + Mathf.Abs(obj.colliderSize.y) > 0.00001f)
                {
                    if(obj.colliderShape =="box"){
                        interactable.GetComponent<InteractionObj>().hideCriteria = obj.colliderOffset.y-obj.colliderSize.y/2-footCollider.offset.y-footCollider.size.y/2;
                    }
                }
        ...
        {
            // InteractionCTRL initializes its dialogue anchor during Start, so wait one frame.
            yield return null;
